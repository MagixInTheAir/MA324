...
public:
	...
	Matrix(std::vector<std::vector<T>> const& _data) : data(_data) {}; // Implemented
	Matrix(size_t const& lines, size_t const& cols) : data(lines, std::vector<T>(cols, T())) {}; // Implemented
	template<class T2> Matrix(Matrix<T2> const& other) : data(other) {}; // Implemented
	template<class T2> Matrix(Matrix<T2>&& other) : Matrix<T>() { swap(*this, other); }; // Implemented
	...
	void setLine(size_t pos, std::vector<T> const& data); // Implemented
	void setCol(size_t pos, std::vector<T> const& data); // Implemented
	void setLine(size_t pos, Matrix<T> const& data); // Implemented
	void setCol(size_t pos, Matrix<T> const& data); // Implemented
	...
	template<class T2> Matrix<T>& operator=(Matrix<T2> other); // Implemented
	...
	T highest_eigenval_iteratedPower(std::vector<T> const& x0, T const& precision, unsigned long long maxiter) const; // Implemented, tested
	T lowest_eigenval_invIteratedPower(std::vector<T> const& x0, T const& precision, unsigned long long maxiter) const; // Implemented, tested
	...
	// OTHERS
	template<class T2> auto dot(Matrix<T2> const& other) const;
	bool isDiagonal() const; // Implemented
	

	// GENERATORS
	static Matrix<T> gen_random(size_t size, T const& min, T const& max); // Implemented
	static Matrix<T> gen_random(size_t lines, size_t cols, T const& min, T const& max); // Implemented
	static Matrix<T> gen_diag(size_t size, T const& value = T()); // Implemented
	static Matrix<T> gen_diag(size_t lines, size_t cols, T const& value = T()); // Implemented
	static Matrix<T> gen_diag(std::vector<T> const& values); // Implemented
	static Matrix<T> gen_full(size_t size, T const& value = T()); // Implemented
	static Matrix<T> gen_full(size_t lines, size_t cols, T const& value = T()); // Implemented
	static Matrix<T> gen_col(std::vector<T> const& values); // Implemented
	static Matrix<T> gen_line(std::vector<T> const& values); // Implemented

	// COMPARATORS
	bool allclose(Matrix<T> const& other, T const& abs_precision, T const& rel_precision) const; // Implemented
	static bool close(T const& lhs, T const& rhs, T const& abs_precision, T const& rel_precision); // Implemented
	static bool allclose(std::vector<T> const& lhs, std::vector<T> const& rhs, T const& abs_precision, T const& rel_precision); // Implemented
