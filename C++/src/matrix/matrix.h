...
template<class T>
Matrix<T> Matrix<T>::gen_random(size_t lines, size_t cols, T const& min, T const& max) {
...
template<class T>
Matrix<T> Matrix<T>::gen_random(size_t size, T const& min, T const& max) { return Matrix<T>::gen_random(size, size, min, max); };

template<class T>
Matrix<T> Matrix<T>::gen_diag(size_t lines, size_t cols, T const& value) {
    Matrix<T> result(lines, cols);
...
template<class T>
Matrix<T> Matrix<T>::gen_diag(size_t size, T const& value) {
    return Matrix<T>::gen_diag(size, size, value);
}

template<class T>
Matrix<T> Matrix<T>::gen_diag(std::vector<T> const& values) {
...
template<class T>
Matrix<T> Matrix<T>::gen_full(size_t lines, size_t cols, T const& value){
	return Matrix<T>(std::vector<std::vector<T>>(lines, std::vector<T>(cols, value)));
};

template<class T>
Matrix<T> Matrix<T>::gen_full(size_t size, T const& value) {
...
template<class T>
template<class T2>
Matrix<T>& Matrix<T>::operator=(Matrix<T2> other) {
	swap(*this, other);
	return *this;
};

template<typename T>
T Matrix<T>::highest_eigenval_iteratedPower(std::vector<T> const& x0, T const& precision, unsigned long long maxiter) const {
...
template<typename T>
T Matrix<T>::lowest_eigenval_invIteratedPower(std::vector<T> const& x0, T const& precision, unsigned long long maxiter) const {
...
template<typename T>
bool Matrix<T>::allclose(Matrix<T> const& other, T const& abs_precision, T const& rel_precision) const {
	if (this->lines() != other.lines() || this->cols() != other.cols()) {
	...
	for (unsigned i = 0; i < other.lines(); i++) {
		for (unsigned j = 0; j < other.cols(); j++) {
			if (!Matrix<T>::close(this->get(i, j), other.get(i,j), abs_precision, rel_precision)) {
				return false;
			}
		}
	}
...
template<class T>
bool Matrix<T>::close(T const& lhs, T const& rhs, T const& abs_precision, T const& rel_precision) {
...
template<class T>
bool Matrix<T>::allclose(std::vector<T> const& lhs, std::vector<T> const& rhs, T const& abs_precision, T const& rel_precision) {
...
template<class T>
Matrix<T> Matrix<T>::gen_col(std::vector<T> const& values) {
...
template<class T>
Matrix<T> Matrix<T>::gen_line(std::vector<T> const& values) {
...
template<typename T>
void Matrix<T>::setLine(size_t pos, std::vector<T> const& data) {
...
template<typename T>
void Matrix<T>::setCol(size_t pos, std::vector<T> const& data) {
...
template<typename T>
void Matrix<T>::setLine(size_t pos, Matrix<T> const& data) {
...
template<typename T>
void Matrix<T>::setCol(size_t pos, Matrix<T> const& other) {
